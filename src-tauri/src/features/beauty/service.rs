...
pub async fn get_popular_presets(
    ...
    // Sort results: wanted presets first, then the rest
    if !wanted_ids.is_empty() {
        result.sort_by(|a, b| {
            let a_wanted = wanted_ids.contains(&a["preset_id"].as_str().unwrap_or("").to_string());
            let b_wanted = wanted_ids.contains(&b["preset_id"].as_str().unwrap_or("").to_string());
            match (b_wanted, a_wanted) {
