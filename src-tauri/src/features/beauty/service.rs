...
pub async fn get_classes_for_presets(db: &DatabaseConnection) -> Result<Vec<serde_json::Value>, AppError> {
    Ok(ClassRepository::get_for_presets(db).await?)
}

pub async fn get_classes_for_popular(db: &DatabaseConnection, since_ts: i64) -> Result<Vec<serde_json::Value>, AppError> {
    Ok(ClassRepository::get_for_popular(db, since_ts).await?)
}
